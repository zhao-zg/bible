...
// 验证方式：URL 含 ?zzg 参数、或 Cookie cx_access=ok
// 放行例外（APK/App 内检查更新与跨域拉取需要）：version.json、changelog.json、
// *.apk、img/zanzhu/*（赞助二维码远程加载）、speedtest.bin（线路竞速测速）
...
function isExempt(path) {
  ...
  // 赞助二维码：App 运行时远程加载（不进 APK、不缓存）
  if (path.startsWith('/img/zanzhu/')) return true;
  // 线路竞速测速文件
  if (path === '/speedtest.bin') return true;
  return false;
}
...
export async function onRequest(context) {
  ...
  // 未验证访客：放行例外资源（APK 更新检查、赞助二维码、测速文件）
  if (isExempt(path)) {
    return next();
  }
