...
(function (win) {
    ...
    function _save(arr) {
        _initStore();
        var savePromise = _store.setItem(STORAGE_KEY, arr);
        // 超时保护：与 _load() 对称，防止 IndexedDB 挂起导致 Promise 永远 pending
        var timeoutPromise = new Promise(function (_, reject) {
            setTimeout(function () {
                reject(new Error('save timeout'));
            }, 3000);
        });
        return Promise.race([savePromise, timeoutPromise]).catch(function (e) {
            console.warn('[书签] 保存超时或失败，降级到 localStorage', e);
            try {
                localStorage.setItem('bible_bookmarks_fallback', JSON.stringify(arr));
            } catch (ex) {}
            // 不抛出错误，让调用方的 .then() 仍然执行
        });
    }
    ...
    var CXBookmark = {
        ...
        goto: function (bookmark) {
            ...
            var checkInterval = setInterval(function () {
                ...
                // 检查 app 容器是否有内容且可见
                if (appEl && appEl.children.length > 0 && appEl.style.display !== 'none') {
