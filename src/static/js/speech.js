...
(function () {
  ...
  function init(options) {
    ...
    // 锁屏/通知栏：title = 篇章标题（大字），artist = 应用名（副标题小字）
    var _pageTitleRaw = document.title || '';
    var title  = (options && options.title)  || _pageTitleRaw  || '朗读';
    ...
    function startInit() {
      ...
      if (useWebSpeech && !_visibilityListenerBound) {
        _visibilityListenerBound = true;
        document.addEventListener('visibilitychange', function () {
          if (document.visibilityState === 'hidden' && _activeResetState) {
            _activeResetState();
          }
        });
      }
