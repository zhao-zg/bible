...
(function() {
    ...
    // 获取主题颜色（从 CSS 变量实时读取当前主题色，确保跟随主题切换）
    function getTheme() {
        var s = getComputedStyle(document.documentElement);
        var brand = s.getPropertyValue('--brand').trim() || '#8B4513';
        var brandRgb = s.getPropertyValue('--brand-rgb').trim() || '139,69,19';
        var success = s.getPropertyValue('--success').trim() || '#6B7F4A';
        return {
            brand: brand,
            brandDark: brand,
            bg: brand,
    ...
    function showApkDownloadProgress(message, progress, speed, downloaded) {
        ...
        var html = '<div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: var(--overlay-strong); z-index: 10001; display: flex; align-items: center; justify-content: center; padding: 20px;" id="' + dialogId + '">';
        html += '<div style="background: var(--surface); border-radius: 12px; padding: 24px; max-width: 400px; width: 100%;">';
        html += '<h3 style="color: ' + THEME.brand + '; margin-bottom: 15px; font-size:0.889rem; text-align: center;">📱 正在下载 APK</h3>';
        html += '<p style="color: var(--text-color); margin-bottom: 10px; text-align: center; font-size:0.741rem;" id="apkProgressMessage">' + message + '</p>';

        html += '<p style="color: var(--text-muted); margin-bottom: 15px; text-align: center; font-size:0.642rem;" id="apkProgressInfo">';
        ...
        html += '<div style="background: var(--surface-alt); border-radius: 10px; height: 20px; overflow: hidden; margin-bottom: 10px;">';
        html += '<div id="apkProgressBar" style="background: ' + THEME.brand + '; height: 100%; width: ' + progress + '%; transition: width 0.3s;"></div>';
        html += '</div>';

        html += '<p style="color: var(--text-muted); text-align: center; font-size:0.642rem;" id="apkProgressPercent">' + progress + '%</p>';
        html += '</div></div>';
    ...
    function showApkUpdateDialog(release, apk, currentVersion, comparison) {
        ...
        if (isVersionUnknown) {
            html += '<h3 style="color: ' + THEME.brand + '; margin-bottom: 15px; font-size:1.111rem;">📱 APK 下载</h3>';
        } else if (comparison > 0) {
            html += '<h3 style="color: ' + THEME.brand + '; margin-bottom: 15px; font-size:1.111rem;">🎉 发现新版本</h3>';
        } else if (comparison === 0) {
            html += '<h3 style="color: ' + THEME.success + '; margin-bottom: 15px; font-size:1.111rem;">✅ 已是最新版本</h3>';
        } else {
            html += '<h3 style="color: ' + THEME.brand + '; margin-bottom: 15px; font-size:1.111rem;">📱 版本信息</h3>';
        }

        html += '<div style="color: var(--text-color); margin-bottom: 20px; font-size:0.741rem; line-height: 1.6;">';
        ...
        if (isVersionUnknown) {
            html += '<div style="background: var(--surface-alt); padding: 10px; border-radius: 8px; font-size:0.689rem; text-align: center; color: var(--warning); margin-bottom: 15px;">';
            html += '⚠️ 无法获取当前版本号<br>建议下载最新版本';
            html += '</div>';
        } else if (comparison === 0) {
            html += '<div style="background: var(--surface-alt); padding: 10px; border-radius: 8px; font-size:0.689rem; text-align: center; color: ' + THEME.success + '; margin-bottom: 15px;">';
            html += '✨ 您使用的已经是最新版本';
            html += '</div>';
        } else if (comparison > 0) {
            html += '<div style="background: var(--surface-alt); padding: 10px; border-radius: 8px; font-size:0.689rem; text-align: center; color: var(--warning); margin-bottom: 15px;">';
        ...
        if (isVersionUnknown || comparison > 0) {
            ...
            html += '<button style="width: 100%; padding: 12px; margin-bottom: 10px; background: ' + THEME.success + '; color: var(--brand-text); border: none; border-radius: 8px; font-size:0.741rem; font-weight: 600; cursor: pointer;" onclick="window.AppUpdate.downloadApkWithUI(\'' + apk.browser_download_url + '\')">';
            html += btnText + sizeText;
            html += '</button>';
        } else {
            html += '<button style="width: 100%; padding: 12px; margin-bottom: 10px; background: ' + THEME.brand + '; color: var(--brand-text); border: none; border-radius: 8px; font-size:0.741rem; font-weight: 600; cursor: pointer;" onclick="window.AppUpdate.downloadApkWithUI(\'' + apk.browser_download_url + '\')">';
            html += '💾 重新下载' + sizeText;
            html += '</button>';
        }

        html += '</div>';
        html += '<button id="apkUpdateDialogCloseBtn" style="width: 100%; padding: 12px; background: var(--surface-alt); color: var(--text-muted); border: none; border-radius: 8px; font-size:0.741rem; font-weight: 600; cursor: pointer;">关闭</button>';
        html += '</div>';
    ...
    function renderSingleVersionHtml(version, entry) {
        ...
        if (entry.date) html += ' <span style="font-weight:400;color:var(--text-muted);font-size:0.593rem;">' + entry.date + '</span>';
        html += '</div>';
        if (entry['new'] && entry['new'].length) {
            html += '<div style="margin-bottom:3px;"><span style="color:var(--success);font-size:0.593rem;font-weight:600;">✨ 新增</span>';
            html += '<ul style="margin:2px 0 0 14px;padding:0;font-size:0.689rem;color:var(--text-color);">';
            entry['new'].forEach(function(item) { html += '<li>' + item + '</li>'; });
            html += '</ul></div>';
        }
        if (entry['opt'] && entry['opt'].length) {
            html += '<div style="margin-bottom:3px;"><span style="color:var(--brand);font-size:0.593rem;font-weight:600;">⚡ 优化</span>';
            html += '<ul style="margin:2px 0 0 14px;padding:0;font-size:0.689rem;color:var(--text-color);">';
            entry['opt'].forEach(function(item) { html += '<li>' + item + '</li>'; });
            html += '</ul></div>';
        }
        if (entry['fix'] && entry['fix'].length) {
            html += '<div style="margin-bottom:3px;"><span style="color:var(--danger);font-size:0.593rem;font-weight:600;">🔧 修复</span>';
            html += '<ul style="margin:2px 0 0 14px;padding:0;font-size:0.689rem;color:var(--text-color);">';
        ...
        if (comparison > 0 && clInline) {
            ...
            if (displayVersions.length > 0) {
                ...
                if (displayVersions.length > 1) {
                    html += '<div style="margin-bottom:8px;font-size:0.593rem;color:var(--text-muted);">本次更新包含以下版本：</div>';
                }
                ...
                clInline.innerHTML = '<div style="color:var(--text-muted);font-size:0.689rem;text-align:center;padding:4px 0;">暂无更新说明</div>';
            }
        ...
        if (histContent && historyVersions.length > 0) {
            ...
            function _renderHistPage() {
                ...
                if (_histShown < historyVersions.length) {
                    ...
                    moreBtn.style.cssText = 'width:100%;padding:0.5rem;background:var(--surface-alt);color:var(--text-muted);border:1px solid var(--border);border-radius:0.333rem;font-size:0.689rem;cursor:pointer;margin-top:0.222rem;';
    ...
    function createUpdateDialog(dialogId, title, statusId, btnId) {
        ...
        var html = '<div style="background:var(--surface);border-radius:12px;max-width:400px;width:100%;max-height:88vh;overflow:hidden;">';

        // ── 主面板 ──
        html += '<div id="' + dialogId + '-panel-main" style="display:block;">';
        html += '<div style="padding:20px 20px 16px;overflow-y:auto;max-height:88vh;">';
        html += '<h3 style="color:' + THEME.brand + ';margin-bottom:14px;font-size:0.889rem;text-align:center;">' + title + '</h3>';
        html += '<div style="padding:14px;background:var(--surface-alt);border-radius:8px;border:1px solid var(--border);margin-bottom:12px;">';
        html += '<div id="' + statusId + '" style="color:var(--text-color);font-size:0.741rem;line-height:1.7;">正在检查...</div>';
        html += '<button id="' + btnId + '" style="display:none;width:100%;padding:10px;margin-top:10px;background:' + THEME.brand + ';color:var(--brand-text);border:none;border-radius:6px;font-size:0.741rem;font-weight:600;cursor:pointer;">立即更新应用</button>';
        html += '</div>';
        html += '<div id="' + dialogId + '-cl-inline" style="display:none;background:var(--surface-alt);border:1px solid var(--border);border-radius:8px;padding:12px 14px;margin-bottom:12px;overflow-y:auto;max-height:260px;font-size:0.689rem;"></div>';
        html += '<button id="' + dialogId + '-hist-btn" style="display:none;width:100%;padding:9px 14px;margin-bottom:12px;background:var(--surface-alt);color:var(--text-muted);border:1px solid var(--border);border-radius:6px;font-size:0.689rem;font-weight:600;cursor:pointer;text-align:left;">📖 历史版本 ›</button>';
        html += '<button id="' + dialogId + '-close" style="width:100%;padding:11px;background:var(--surface-alt);color:var(--text-muted);border:none;border-radius:8px;font-size:0.741rem;font-weight:600;cursor:pointer;">关闭</button>';
        ...
        html += '<button id="' + dialogId + '-hist-back" style="background:none;border:none;color:' + THEME.brand + ';font-size:0.689rem;font-weight:600;cursor:pointer;padding:4px 10px 4px 0;">← 返回</button>';
        html += '<span style="font-size:0.79rem;font-weight:600;color:var(--heading);">📖 历史版本</span>';
        html += '</div>';
        html += '<div id="' + dialogId + '-hist-content" style="padding:14px 16px;overflow-y:auto;max-height:calc(88vh - 50px);font-size:0.689rem;"></div>';
        ...
        getCurrentApkVersion().then(function(currentVersion) {
            ...
            return window.CX.raceFastest(urls, {
                ...
                if (comparison > 0) {
                    ...
                    if (clInline) {
                        clInline.style.display = 'block';
                        clInline.innerHTML = '<div style="color:var(--text-muted);font-size:0.689rem;text-align:center;padding:4px 0;">📋 正在加载更新内容...</div>';
                    }
                }
