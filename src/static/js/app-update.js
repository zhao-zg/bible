...
(function() {
    ...
    function showApkDownloadProgress(message, progress, speed, downloaded) {
        ...
        html += '<p style="color: var(--text-color); margin-bottom: 10px; text-align: center; font-size:14px;" id="apkProgressMessage">' + message + '</p>';

        html += '<p style="color: var(--text-muted); margin-bottom: 15px; text-align: center; font-size:14px;" id="apkProgressInfo">';
        ...
        html += '<p style="color: var(--text-muted); text-align: center; font-size:14px;" id="apkProgressPercent">' + progress + '%</p>';
        html += '</div></div>';
    ...
    function showApkUpdateDialog(release, apk, currentVersion, comparison) {
        ...
        html += '<div style="color: var(--text-color); margin-bottom: 20px; font-size:14px; line-height: 1.6;">';
        ...
        if (isVersionUnknown) {
            html += '<div style="background: var(--surface-alt); padding: 10px; border-radius: 8px; font-size:14px; text-align: center; color: var(--warning); margin-bottom: 15px;">';
            html += '⚠️ 无法获取当前版本号<br>建议下载最新版本';
            html += '</div>';
        } else if (comparison === 0) {
            html += '<div style="background: var(--surface-alt); padding: 10px; border-radius: 8px; font-size:14px; text-align: center; color: ' + THEME.success + '; margin-bottom: 15px;">';
            html += '✨ 您使用的已经是最新版本';
            html += '</div>';
        } else if (comparison > 0) {
            html += '<div style="background: var(--surface-alt); padding: 10px; border-radius: 8px; font-size:14px; text-align: center; color: var(--warning); margin-bottom: 15px;">';
    ...
    function renderSingleVersionHtml(version, entry) {
        ...
        if (entry.date) html += ' <span style="font-weight:400;color:var(--text-muted);font-size:14px;">' + entry.date + '</span>';
        html += '</div>';
        if (entry['new'] && entry['new'].length) {
            html += '<div style="margin-bottom:3px;"><span style="color:var(--success);font-size:14px;font-weight:600;">✨ 新增</span>';
            html += '<ul style="margin:2px 0 0 14px;padding:0;font-size:14px;color:var(--text-color);">';
            entry['new'].forEach(function(item) { html += '<li>' + item + '</li>'; });
            html += '</ul></div>';
        }
        if (entry['opt'] && entry['opt'].length) {
            html += '<div style="margin-bottom:3px;"><span style="color:var(--brand);font-size:14px;font-weight:600;">⚡ 优化</span>';
            html += '<ul style="margin:2px 0 0 14px;padding:0;font-size:14px;color:var(--text-color);">';
            entry['opt'].forEach(function(item) { html += '<li>' + item + '</li>'; });
            html += '</ul></div>';
        }
        if (entry['fix'] && entry['fix'].length) {
            html += '<div style="margin-bottom:3px;"><span style="color:var(--danger);font-size:14px;font-weight:600;">🔧 修复</span>';
            html += '<ul style="margin:2px 0 0 14px;padding:0;font-size:14px;color:var(--text-color);">';
        ...
        if (comparison > 0 && clInline) {
            ...
            if (displayVersions.length > 0) {
                ...
                if (displayVersions.length > 1) {
                    html += '<div style="margin-bottom:8px;font-size:14px;color:var(--text-muted);">本次更新包含以下版本：</div>';
                }
                ...
                clInline.innerHTML = '<div style="color:var(--text-muted);font-size:14px;text-align:center;padding:4px 0;">暂无更新说明</div>';
            }
    ...
    function createUpdateDialog(dialogId, title, statusId, btnId) {
        ...
        html += '<div id="' + statusId + '" style="color:var(--text-color);font-size:14px;line-height:1.7;">正在检查...</div>';
        ...
        html += '</div>';
        html += '<div id="' + dialogId + '-cl-inline" style="display:none;background:var(--surface-alt);border:1px solid var(--border);border-radius:8px;padding:12px 14px;margin-bottom:12px;overflow-y:auto;max-height:260px;font-size:14px;"></div>';
        ...
        html += '</div>';
        html += '<div id="' + dialogId + '-hist-content" style="padding:14px 16px;overflow-y:auto;max-height:calc(88vh - 50px);font-size:14px;"></div>';
        ...
        getCurrentApkVersion().then(function(currentVersion) {
            ...
            return window.CX.cxFetch(urls, {
                ...
                if (comparison > 0) {
                    ...
                    if (clInline) {
                        clInline.style.display = 'block';
                        clInline.innerHTML = '<div style="color:var(--text-muted);font-size:14px;text-align:center;padding:4px 0;">📋 正在加载更新内容...</div>';
                    }
                }
