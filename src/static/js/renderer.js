...
(function (win) {
  ...
  // 朗读控制栏已重构为弹窗模式（#speechDialog in index.html），不再内联渲染
  ...
  function initDayPager(initialPage) {
    ...
    if (_dayPagerKeyHandler) {
      document.removeEventListener('keydown', _dayPagerKeyHandler);
    }
    _dayPagerKeyHandler = function(e){
  ...
  function initSpeechForView(viewType, chapter) {
    function tryInit() {
      ...
      var dialog = document.getElementById('speechDialog');
      var btn = document.getElementById('playPauseBtn');
      if (!dialog || !btn) return true;
    ...
    loadTraining(batchPath)
      ...
      .catch(function(err) {
        ...
        var packHint = isCapacitor
          ? '<p style="font-size:0.642rem;color:var(--text-secondary)">此训练可能需要下载历史资源包</p>' +
